...
class MainWindow(QtWidgets.QMainWindow, MainForm.Ui_MainWindow):
    ...
    def saveRecord(self):
        ...
        if table_name == 'contractss':
            ...
            if not self.validateContractInput(name, code, date):

                QtWidgets.QMessageBox.critical(self, "Ошибка ввода",
                                               "Некорректные данные для контракта. Пожалуйста, проверьте введенные значения.")
                return
            ...
            start_date = self.start_dateEdit.date().toString("dd-MMM-yy")

            day_end = self.dateEdit.date().toString("dd-MMM-yy")
            ...
            if not self.validateStatInput(name, start_date, day_end, price, min_price, max_price, quant):

                QtWidgets.QMessageBox.critical(self, "Ошибка ввода",
                                               "Некорректные данные для статистики. Пожалуйста, проверьте введенные значения.")
        ...
        if self.currentRow is None:
            ...
            if table_name == 'contractss':
                ...
                self.model.insertRow(0, [QStandardItem(str(new_id)), QStandardItem(name), QStandardItem(code), QStandardItem(date)])

            elif table_name == 'stat':

                cursor.execute(
                    f"INSERT INTO {table_name} (name, start_date, day_end, price, min_price, max_price, contracts_quantity) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (name, start_date, day_end, price, min_price, max_price, quant))

                new_id = cursor.lastrowid
                self.model.insertRow(0, [QStandardItem(str(new_id)), QStandardItem(name), QStandardItem(start_date),
                                         QStandardItem(day_end),
                                                                       QStandardItem(price), QStandardItem(min_price),
                                         QStandardItem(max_price), QStandardItem(quant)])
    ...
    def validateContractInput(self, name, code, date):


        if not name or not code or not date:
    ...
    def validateStatInput(self, name, start_date, day_end, price, min_price, max_price, quant):
        # Add your validation logic for statistics inputs

        if not name or not price or not quant:
            return False
        return True
    ...
    def load_table_from_db(self, table_name, db_name):
        ...
        if table_name == 'summary':
            self.toggleButtons(False)  # Hide buttons for the summary page
        else:
